...
.controller('rootController', [
    ...
    function($rootScope, $scope, $location, $http, $uibModal) {
		...
		$http({
            method: 'GET',
            url: '/index.php?get=core_config_types'
        })
        .then(
		...
		$http({
            method: 'GET',
            url: '/index.php?get=core_config_classes'
        })
        .then(
            function success(json) {
		...
		$http({
			method: 'GET',
			url: '/index.php?get=core_config_classes&package='+global_config['package']
		})
		.then(
			function success(json) {				
				...
				angular.forEach(json.data, function (cls, key) {
					...
					$http({
						method: 'GET',
						url: '/index.php?get=core_model_schema&entity='+global_config['package']+'\\'+cls
					})
					.then(
						function success(json) {
		...
		$scope.save = function() {
			...
			modalInstance.result.then(
				function () {
					$http.post('index.php?do=core_config_save-model', {
						schema: class_def,
