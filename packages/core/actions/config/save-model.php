...
if($params['part'] == 'class') {
    ...
    // Create a temporary file with the following contents and then parse it in order to generate a minimal AST
...
function sanitizeColumns($properties) {
    ...
    foreach($properties as $property => $descriptor) {
        if(array_key_exists($property, $special_columns)){
            if($descriptor != $special_columns[$property]){
                if($special_columns[$property]["type"] != 'datetime') {
