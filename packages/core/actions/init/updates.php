...
[$params, $providers] = eQual::announce([
    ...
    'help'          => 'If no package given, then list the pending updates of each packages.',
    'params'        => [
        'package' => [
...
$action = !empty($params['package']) ? 'execute' : 'list';

if($action === 'execute' && !in_array($params['package'], $packages, true)) {
    throw new Exception('unknown_package', EQ_ERROR_INVALID_PARAM);
}

$result = [];
foreach($packages as $package) {
    if($action === 'execute' && $package !== $params['package']) {
        continue;
    }
    ...
    if(is_dir($updates_folder)) {
        foreach(glob($updates_folder . '/*.php') as $filename) {
            ...
            if(
                ...
                $result[$package][$action === 'execute' ? 'skipped' : 'skip'][] = $script;
                continue;
            }
    ...
    foreach($update_scripts as $update_script) {
        ...
        if($action === 'execute') {
            ...
            $result[$package]['executed'][] = $script;
        }
        else {
            $result[$package]['pending'][] = $script;
        }
    }
}
