...
Before ACLs are evaluated, generic `Collection` CRUD operations can be structurally blocked or limited by [Entity Capabilities](../entities-persistence/entities.md#capabilities). ACLs cannot grant an operation that the entity does not expose through its capabilities: `getCapabilities()` acts as a restrictive gate, so an entity may explicitly refuse an operation even when ACLs grant the matching right.
