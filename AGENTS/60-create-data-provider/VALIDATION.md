...
Validate the data provider definition through eQual's `announce` mechanism. No dedicated provider-consistency controller currently covers provider announcement metadata, so this is an allowed `core_json-validate` exception.
...
2. Do not parse the PHP data provider file manually.
3. Ask eQual for the `eQual::announce()` metadata through `./equal.run --get={controller} --announce=true`; this returns the provider contract without executing the retrieval logic after `eQual::announce()`.
