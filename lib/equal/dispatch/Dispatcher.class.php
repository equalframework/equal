...
class Dispatcher extends Service {
     ...
     * 
     */
    public function dispatch($message_model, $object_class, $object_id, $severity='notice', $controller=null, $params="{}") {
        $orm = $this->container->get('orm');
        trigger_error("dispatching message", E_USER_WARNING);
     ...
     * The message if deleted and, if it relates to a controller, a call is made (which, in turn, can lead to the message being re-created).
     *
    ...
    public function dismiss($id) {
        ...
        if($messages_ids > 0) {
            $message_id = reset($messages);
            $message = $orm->read('core\alert\Message', $message_id, ['controller', 'params']);
