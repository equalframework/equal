    ...
    */
    ...
    private static function conditionCheck($condition, $schema=[]) {
        ...
        if(!is_array($condition)) {
            trigger_error("QN_M_ORM::condition is not an array", QN_REPORT_DEBUG);
            return false;
        }
        // condition must be composed of 3 elements (field, operator, operand)
        if(count($condition) != 3) {
            trigger_error("QN_M_ORM::missing condition in domain", QN_REPORT_DEBUG);
            return false;
        }
        ...
        if(!empty($schema)) {
            ...
            if(!in_array($field, array_keys($schema))) {
                trigger_error("QN_M_ORM::unknown field '{$field}' in domain", QN_REPORT_DEBUG);
                return false;
            }
            // handle 'alias'
            $is_alias = false;
            ...
            if(!in_array($operator, ObjectManager::$valid_operators[$target_type])) {
                trigger_error("QN_M_ORM::invalid operator '{$operator}' in domain", QN_REPORT_DEBUG);
                return false;
            }
        }
        return true;
