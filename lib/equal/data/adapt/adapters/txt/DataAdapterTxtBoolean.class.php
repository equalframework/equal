...
class DataAdapterTxtBoolean implements DataAdapter {
    ...
    public function adaptOut($value, $usage, $locale='en') {
        $result = '';
        if($value) {
            $result = Locale::get_term('core', 'bool.true', 'true', $locale);
        }
        else {
            $result = Locale::get_term('core', 'bool.false', 'false', $locale);
        }
        return $result;
    }
